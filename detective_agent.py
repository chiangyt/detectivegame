...
Uses Claude's native tool use to drive a two-phase agentic loop:
...
# Human-readable hints used only in prompts — keys must match condition names exactly
CONDITION_HINTS = {
    "Daniel的眼镜腿是弯的":     "现场眼镜完好无损，但Jordan提过Daniel的眼镜腿弯了 → 为什么现场遗留的眼镜是完好的？难道被掉包了？",
    "Morgan平时戴眼镜":         "Lucas说Morgan平时会戴眼镜，但案发当天没戴 → 为什么刻意不戴？还是说发生了什么让他的眼镜无法戴上？",
    "Morgan今天没戴眼镜":       "Morgan声称眼镜忘在家里 → 结合平时必须戴眼镜，看来有重大嫌疑。",
    "Morgan突然签了合同":       "Morgan在压力下签了搬迁补贴协议 → 背后存在经济动机",
    "Morgan和Daniel有经济纠纷": "Daniel以贷款陷阱骗Morgan签合同 → 直接的杀人动机",
}
...
class DetectiveAgent:
    def __init__(self, agent_manager, script_mgr, evidence_handler, anthropic_client, model="claude-sonnet-4-6"):
        ...
        self.model                = model
        self.max_turns             = 25
        self.crime_process_deduced = False  # unlocked after deduce_crime_process succeeds
        self.deduced_location      = None   # location chosen in deduce_crime_process
        self._accusation_accepted  = False  # set by _execute_tool, read by _tool_events
        ...
        with ls_trace("detective_investigation", run_type="chain"):
            for turn in range(self.max_turns):
                ...
                for block in response.content:
                    ...
                    result_content, stop = self._execute_tool(block)
                    tool_results.append({
                        "type":        "tool_result",
                    ...
                    for event in self._tool_events(block, turn + 1):
                        yield event

                # Append a single per-turn conditions summary after all tool results
                ...
                if evidence_done and not motive_done:
                    summary += "\n\n💡 物证已查明，建议转向动机：询问嫌疑人 Morgan 与死者的矛盾。"
                ...
                messages.append({
                    "role": "user",
                    "content": tool_results + [{"type": "text", "text": summary}],
                })
    ...
    def _execute_tool(self, block):
        """Execute a tool_use block. Returns (result_str, should_stop)."""
        name = block.name
        inp  = block.input
        ...
        if name == "deduce_crime_process":
            ...
            if not evidence_done:
                ...
                missing_text = "\n".join(
                    f"  - 「{c}」（{CONDITION_HINTS.get(c, '')}）" for c in missing
                )
                return (
                    f"❌ 推断被驳回：物证线索不足，还缺：\n{missing_text}\n请继续调查物证。"
                ), False
            ...
            return "✅ 作案过程推断已确认。现在可以提交最终指控（make_accusation）。", False
    ...
    def _tool_events(self, block, turn):
